...
void print_gps_data(const Xense_gps &data) {
    std::cout << "Latitude: " << data.lat_deg << "." << data.lat_billionths << " degrees" << std::endl;
    std::cout << "Longitude: " << data.lon_deg << "." << data.lon_billionths << " degrees" << std::endl;
...
void test_xense_gps_serialization_deserialization() {
    ...
    // Serialize
    XenseStatus status = serialize_xense_gps(gps_data, buffer, &buffer_size, XENSE_GPS_TYPE_ID);
    ...
    status = deserialize_xense_gps(buffer, buffer_size, deserialized_gps_data, nullptr);
    ...
    std::cout << "Xense GPS serialization and deserialization test passed!" << std::endl;
}
