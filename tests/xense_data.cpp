...
void print_data(const Xense_data &data)
{
    ...
    std::cout << "Is charging: " << (data.is_charging ? "true" : "false") << std::endl;
}
...
void test_xense_data_serialization_deserialization()
{
    ...
    // Serialize
    XenseStatus status = serialize_xense_data(data, buffer, &buffer_size, XENSE_DATA_TYPE_ID);
    ...
    status = deserialize_xense_data(buffer, buffer_size, deserialized_data, nullptr);
