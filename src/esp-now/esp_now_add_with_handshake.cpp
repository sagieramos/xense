 ...
 */
 ...
 static void handshake_send_cb(const uint8_t *mac_addr, esp_now_send_status_t status);
 static void handshake_recv_cb(const esp_now_recv_info_t *esp_now_info, const uint8_t *data, int data_len);
 ...
 esp_err_t esp_now_add_peer_with_handshake(const esp_now_peer_info_t *peer) {
     ...
     pending_handshake->timer = xTimerCreate(
         "handshake_timer",
 ...
 static void handshake_recv_cb(const esp_now_recv_info_t *esp_now_info, const uint8_t *data, int data_len) {
     ...
     if (pending_handshake != NULL && data_len >= sizeof(handshake_msg_t)) {
         ...
         if (msg->type == HANDSHAKE_MSG_TYPE && 
             memcmp(esp_now_info->src_addr, pending_handshake->peer_addr, ESP_NOW_ETH_ALEN) == 0) {
             
             if (msg->status == 0x02) {  // ACK
                ESP_LOGI(TAG,
                    "Handshake completed with peer: %02X:%02X:%02X:%02X:%02X:%02X",
                    pending_handshake->peer_addr[0],
                    pending_handshake->peer_addr[1],
                    pending_handshake->peer_addr[2],
                    pending_handshake->peer_addr[3],
                    pending_handshake->peer_addr[4],
                    pending_handshake->peer_addr[5]);
                 ...
                 esp_err_t ret = esp_now_send(esp_now_info->src_addr, (const uint8_t *)&response, sizeof(response));
                 if (ret != ESP_OK) {
 ...
 static void handshake_timeout_cb(TimerHandle_t timer) {
     if (pending_handshake != NULL) {
         if (!pending_handshake->handshake_complete) {
            ESP_LOGW(TAG,
                "Handshake timeout with peer: %02X:%02X:%02X:%02X:%02X:%02X",
                pending_handshake->peer_addr[0],
                pending_handshake->peer_addr[1],
                pending_handshake->peer_addr[2],
                pending_handshake->peer_addr[3],
