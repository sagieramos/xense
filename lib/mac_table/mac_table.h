...
/**
 * @enum SlotState
 * @brief Describes the state of a table entry.
 */
typedef enum {
  SLOT_EMPTY,     /**< Slot is empty */
  SLOT_TOMBSTONE, /**< Slot was occupied but now deleted */
  SLOT_OCCUPIED   /**< Slot contains a valid MAC address */
} SlotState;

/**
 * @enum MacTableResult
 * @brief Describes return values for MAC table operations.
 */
typedef enum {
  MAC_TABLE_OK,        /**< Operation successful */
  MAC_TABLE_TIMEOUT,   /**< Entry exists but has timed out */
  MAC_TABLE_NOT_FOUND, /**< Entry not found */
  MAC_TABLE_INSERTED,  /**< New entry inserted */
  MAC_TABLE_UPDATED,   /**< Existing entry updated */
  MAC_TABLE_DELETED,   /**< Entry deleted */
  MAC_TABLE_FULL       /**< Table is full; cannot insert */
} MacTableResult;

/**
 * @struct MacTableEntry
 * @brief Represents a single MAC address entry in the table.
 */
typedef struct {
...
/**
 * @brief Callback for table events like insert, update, and delete.
 *
 * @param slot_index Index of the affected slot
 * @param status     Type of event (e.g., MAC
